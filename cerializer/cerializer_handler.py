...
class Cerializer:
	...
	def update_code(self):
		...
		for schema_path, schema_identifier in iterate_over_schema_roots(self.schema_roots):
			...
			self.code[schema_identifier] = self.get_compiled_code(schema, schema_identifier)
		for item in fastavro._schema_common.SCHEMA_DEFS.items():
			print(item)
		print('common.Time:1' in fastavro._schema_common.SCHEMA_DEFS)

	# TODO remove schema identifier = debugging purposes
	def get_compiled_code(self, schema, schema_identifier):

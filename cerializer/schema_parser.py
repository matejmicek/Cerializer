...
def prepare(logical_type, data_type, buffer_name, location, schema):
	if logical_type == 'decimal':
		...
		return f'{constants.constants.WRITE_PREFIX}write.write_{data_type}({buffer_name}, {constants.constants.PREPARE_PREFIX}prepare.prepare_{data_type}_{logical_type}({location}, {params}))'
	return f'{constants.constants.WRITE_PREFIX}write.write_{data_type}({buffer_name}, {constants.constants.PREPARE_PREFIX}prepare.prepare_{logical_type}({location}))'
...
def generate_serialization_code(schema, location, buffer_name: str, jinja_env):
	...
	if f'logicalType' in schema:
		prepared = prepare(schema['logicalType'].replace('-', '_'), type_, buffer_name, location, schema)
		return prepared
	elif type_ == constants.constants.RECORD:
		return '\n'.join((generate_serialization_code(field, location, buffer_name, jinja_env)) for field in schema['fields'])
		...
		name = schema['name']
		new_location = f'{location}[\'{name}\']'
		return generate_serialization_code(type_, new_location, buffer_name, jinja_env)
	...
	# TODO needs to be fixed
		...
		name = schema['name']
		new_location = f'{location}[\'{name}\']'
		return generate_serialization_code(dict(type_), new_location, buffer_name, jinja_env)
